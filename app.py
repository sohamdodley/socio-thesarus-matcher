...
if st.button("Match Keywords with TheSoz", type="primary"):
    ...
    for section_name, keywords in spider_sections.items():
        if not keywords:
            continue
        ...
        if section_matches:
            ...
            # Collect unique preferred terms for Boolean
            prefs = list(dict.fromkeys([m["preferred_term"] for m in section_matches]))
            boolean_parts[section_name] = prefs

    if not all_results:
        st.warning("No good matches found. Try lowering the minimum score or using different keywords.")
    else:
        ...
        # -----------------------------
        st.header("3. Suggested Boolean String")

        boolean_groups = []
        for section, terms in boolean_parts.items():
            if terms:
                group = " OR ".join([f'"{t}"' for t in terms])
                boolean_groups.append(f"({group})")

        final_boolean = " AND ".join(boolean_groups)

        st.code(final_boolean, language="text")

        st.download_button(
            label="Download Mapping as CSV",
            data=pd.concat([pd.DataFrame(m).assign(Section=s) for s, m in all_results]).to_csv(index=False),
            file_name="thesoz_spider_mapping.csv",
            mime="text/csv"
        )
